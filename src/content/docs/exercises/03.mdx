...
1. Go to the `_quote.scss` SCSS partial. We'll definitely need USWDS tokens, so lets add that first.
...
1. Next, lets add the component to our `index.scss` so we can see changes.
...
1. In this step, we'll start adding tokens.
...
1. Next, let's add some typography styles. We know the quote itself is bigger than the other text elements.
    ...
    The table on the [size token docs](https://designsystem.digital.gov/design-tokens/typesetting/font-size/#using-size-tokens-2) show the function expects a family and size to be specified.
...
1. Before we move on take a look at the CSS for the quote marks. We have them as pseudoelements in CSS.
...
1. The quotes are pretty big, so try  `xl` for the quotes.
...
1. Finally lets adjust the author information.
...
1. Next, lets adjust our quote footer by increasing the font size and spacing.
    ...
    <details>
      <summary>Final stylesheet</summary>
