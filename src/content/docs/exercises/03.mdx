...
Below we a common quote component. Although it may appear simple, we're going to apply system level thinking and USWDS design language to extend USWSD and create our own custom component.
...
- The [USWDS Design Tokens page](https://designsystem.digital.gov/design-tokens/) to quickly grab tokens we need
- The [Settings page](https://designsystem.digital.gov/documentation/settings/) to make sure we're following existing patterns.
...
1. In your browser, go to the [Default Quote](http://localhost:6060/?path=/story/components-quote--default) in Storybook. This is where we'll see the preview of the component as we develop.

1. In your code editor, open `quote.html.twig`. Right now its pretty bare bones and is based off of the MDN example. We'll need to add our classes.
...
1. Before we add basic styles, resize your browser to `320px` or enable the mobile view in Storybook.
    ...
    <Aside type="note">
      Not only do we work mobile-first, but this size also covers the WCAG 2.1 AA criteria for reflow!
    </Aside>
...
1. Next, let's add some typography styles. We know the quote itself is bigger than the other text elements.
    ...
    This is body text and the next level up from `md` is `lg`. We'll also remove the margin. For consistency, and to avoid any future confusion we'll stick to units too.
...
1. The quotes are pretty big, so if our quote is `lg`, thanks to our design language we can jump  `3xl` for the quotes knowing that it'll look good.
...
1. Let's quickly move the end quote over to the right.


1. Next, lets add a border and some spacing to the footer.
...
1. Now that we have our mobile styles more or less done, let's move up to tablet.
    ...
    It tells us the values, but for breakpoints the [settings page](https://designsystem.digital.gov/documentation/settings/#utilities-settings-2:~:text=includes%20all%20utilities.-,Utility%20breakpoints,-%24theme%2Dutility%2Dbreakpoints) or
    the [layout grid utilities](https://designsystem.digital.gov/utilities/layout-grid/#responsive-variants-2) which
    settings you have available by default.
