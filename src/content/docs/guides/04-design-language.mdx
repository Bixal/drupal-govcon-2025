...
Next, we'll see how you can apply these ideas using USWDS themes. This is especially helpful if you're supporting multiple sites.
...
Then create a primary color palette (primary, accent, secondary, base). Use these to define component and state colors, and to ensure accessible combinations.
...
3. **Semantic theme and state tokens:**
   ...
   It's a delicate balance to know which one to use and when. Use theme tokens as much as you can so users can easily make high-level changes without having to override multiple smaller settings.
...
Spacing includes margins, padding, and layout/grid systems. Consistent spacing makes layouts clean and familiar. Having a system for spacing is important to have a cohesive design.
